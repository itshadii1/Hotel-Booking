...
function SearchComponent() {
  ...
  const handleGuestSelection = () => {
    setGuests(`${rooms} Room${rooms > 1 ? 's' : ''}, ${adults + children} Guest${adults + children > 1 ? 's' : ''}`);
    setIsGuestPopupOpen(false);
  };

  return (
    <div className="flex justify-center items-center p-6 bg-teal-500 bg-opacity-50 rounded-lg shadow-lg">
      <input
        type="text"
      ...
      <div className="relative p-2 border border-gray-300 rounded-md mr-2 bg-white cursor-pointer" onClick={() => setIsGuestPopupOpen(!isGuestPopupOpen)}>
        {guests}
        {isGuestPopupOpen && (
          <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-gray-300 rounded-md shadow-lg p-4 z-10">
            <div className="flex justify-between mb-2">
              <span>Rooms</span>
              <input type="number" min="1" value={rooms} onChange={(e) => setRooms(Number(e.target.value))} className="w-12 text-center border rounded" />
            </div>
            <div className="flex justify-between mb-2">
              <span>Adults</span>
              <input type="number" min="1" value={adults} onChange={(e) => setAdults(Number(e.target.value))} className="w-12 text-center border rounded" />
            </div>
            <div className="flex justify-between mb-2">
              <span>Children</span>
              <input type="number" min="0" value={children} onChange={(e) => setChildren(Number(e.target.value))} className="w-12 text-center border rounded" />
            </div>
            <button onClick={handleGuestSelection} className="bg-blue-500 text-white px-4 py-2 rounded mt-2 w-full">
              Done
            </button>
          </div>
        )}
